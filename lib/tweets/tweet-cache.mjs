...
export async function preloadTweets(tweetIds, forceUpdate = false) {
  ...
  } else if (!fs.existsSync(CACHE_FILE) && Object.keys(cache.tweets).length > 0) {
    // Defensive: guarantee a cache file exists for the build even when
    // change-detection found nothing new (e.g. fresh dir, deterministic mock data).
    cache.timestamp = Date.now();
    saveTweetCache(cache, allTweetsLoaded);
  }
