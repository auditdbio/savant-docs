...
const AnalysisDemo = () => {
  ...
  const resizeCanvas = useCallback(() => {
    // Compute the height directly (don't read currentDemoContainerHeight state)
    // so this callback's identity stays stable and effects keyed on it don't loop.
    const containerHeight = getCurrentDemoContainerHeight();
    if (canvasRef.current && canvasRef.current.parentElement) {
      const parent = canvasRef.current.parentElement;
      setCanvasDimensions({
        width: parent.clientWidth,
        height: containerHeight > 500 ? 500 : containerHeight,
      });
    }
    ...
    setCurrentDemoContainerHeight(containerHeight);
    ...
    if (tokenizedLines) {
      for (let i = 0; i < tokenizedLines.length; i++) {
        ...
        for (const token of lineTokens) {
          ...
          const tokenColor =
            typeof token.htmlStyle === "object" ? token.htmlStyle?.color : undefined;
          ctx.fillStyle = tokenColor || CODE_TEXT_COLOR;
  ...
  useEffect(() => {
    ...
    // NOTE: pinnedVulnerabilityPanels is intentionally excluded — the functional
    // update reads the latest value, and including it would re-trigger this effect
    // every render (new object refs) causing an infinite update loop.
  }, [scrollY, blocks]);
